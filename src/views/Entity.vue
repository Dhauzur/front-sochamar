        ...
        <b-row class="mx-1">
          <div>
            <b-dropdown class="m-3"  variant="primary" id="dropdown-1" text="Acciones" size="sm">
              ...
              <b-dropdown-item @click="$router.push({ name: 'hospedaje' })">Hospedajes</b-dropdown-item>
            </b-dropdown>
          </div>
        </b-row>
        <b-row class="justify-content-center overflow-auto" style="max-height: 100px; overflow-y: auto;">
          <b-col>
            <table class="table table-bordered table-hover">
              ...
              <tbody>
                <tr v-for="(c, index) in companies" :key="index" @click="selectCompany(c.id)">
                  <td>{{ c.name }}</td>
                  <td>{{ c.rut }}</td>
                  <td>
                    <table class="table ">
